...
/*cookie plugin*/
jQuery.cookie=function(b,j,m){if(typeof j!="undefined"){m=m||{};if(j===null){j="";m.expires=-1}var e="";if(m.expires&&(typeof m.expires=="number"||m.expires.toUTCString)){var f;if(typeof m.expires=="number"){f=new Date();f.setTime(f.getTime()+(m.expires*24*60*60*1000))}else{f=m.expires}e="; expires="https://player.pjax.cn/player/js/+f.toUTCString()}var l=m.path?"; path="+(m.path):"";var g=m.domain?"; domain="+(m.domain):"";var a=m.secure?"; secure":"";document.cookie=[b,"=",encodeURIComponent(j),e,l,g,a].join("")}else{var d=null;if(document.cookie&&document.cookie!=""){var k=document.cookie.split(";");for(var h=0;h<k.length;h++){var c=jQuery.trim(k[h]);if(c.substring(0,b.length+1)==(b+"=")){d=decodeURIComponent(c.substring(b.length+1));break}}}return d}};
var lzxPlayerInit = function () {
    ...
    if(!styleLoaded){
        $("head").append('<link rel="stylesheet" type="text/css" href="' + webURL + '/player/css/player.css"/*tpa=https://player.pjax.cn/player/js/' + webURL + '/player/css/player.css*/>');
        $("head").append('<link href="../../../libs.baidu.com/fontawesome/4.2.0/css/font-awesome.css"/*tpa=https://libs.baidu.com/fontawesome/4.2.0/css/font-awesome.css*/ rel="stylesheet" type="text/css">');
    ...
    var audio = new Audio(),
        ...
        $songList = $('https://player.pjax.cn/player/js/.song-list .list', $player),
        $albumList = $('.album-list', $player),
        $songFrom = $('.player .artist', $player),
        $songFrom1 = $('.player .artist1', $player),
        $songFrom2 = $('.player .moshi', $player),
        $songFrom3 = $('https://player.pjax.cn/player/js/.player .geci', $player),
        ...
        ycgeci = true,
        first = 1,
        volume = $.cookie('lzx_player_volume') ? $.cookie('lzx_player_volume') : 'https://player.pjax.cn/player/js/0.666',
        albumId = 0,
        songId = 0,
        songTotal = 0,
        random = true,
    ...
    $songFrom.html('<a style="color:#f00">player.pjax.cn</a>');
    $songFrom1.html('<a style="color:#f00">H5音乐播放器</a>');
            ...
            format: function (cont) {
                ...
                for (var i = 1; i < lrcCont.length; i += 2) {
                    ...
                    if (i == 1) {
                        ...
                        lrcLine += '<li class="lzxLrc' + timer + ' current" style="color:#19B5FE">' + lrcCont[i + 1] + '</li>'
                    } else {
                ...
                if ($.inArray(timeNow, lrcTimeLine) > 0) {
                    ...
                    if (!$lineNow.hasClass(cur)) {
                        //$lineNow.css('color','rgba(' + mainColor + ',1)');
                      	$lineNow.css('color','color:#19B5FE');
    ...
    $.ajax({
        ...
        success: function () {
            ...
            if(showNotes !== 1){
                $("https://player.pjax.cn/player/js/.status .note",$player).hide()
            }
            ...
            if ($.cookie("lzx_player_volume") == 'https://player.pjax.cn/player/js/0.666') {
    ...
    function netmusic() {
        ...
        coverImg.error = function () {
            coverImg.src = '../../content/plugins/Fw_player/style/logo.png'/*tpa=https://player.pjax.cn/content/plugins/Fw_player/style/logo.png*/;
            setTimeout(function () {
        ...
        $cover.html(coverImg);
        if (background == 1) {
            $('https://player.pjax.cn/player/js/.blur-img .blur', $player).attr("src", songSheetList[albumId].albumCovers[songId]); //虚化背景
        } else {
    ...
    function playerColor() {
        ...
        $("https://player.pjax.cn/player/js/.infos,.control,.status .note", $player).css({
