...
console.log("%c \u8fd9\u91cc\u6709\u6211\u7684\u8054\u7cfb\u65b9\u5f0f(o^.^o) https://pjax.cn/about.html", "color:#00cc00");
...
$(document).on('submit', 'form:not(#commentform,#input,#pay_f)', function (event) {$.pjax.submit(event, pjax_id, {fragment:pjax_id, timeout:6000});}); 
    ...
    $(document).on('pjax:complete', function() {
		  ...
		  $("https://pjax.cn/content/templates/FLY/js/img.lazy").lazyload({effect: "fadeIn"});
		  if(typeof($('#wowslider-container1')[0])!=='undefined'){$.getScript(''+ pjaxtheme +'js/slider.js');};
		  if($("pre").hasClass('sh_xml')){sh_highlightDocument('../content/plugins/SHJS_for_Emlog/lang/', '.min.js'/*tpa=https://pjax.cn/content/templates/FLY/js/.min.js*/);};
...
$('img[src*="https://pjax.cn/content/templates/FLY/js/checkcode.php"]').attr('title', '单击刷新验证码').click(function(){this.src = this.src.replace(/\?.*$/, "") +'?'+ new Date().getTime();});
...
$('img[src*="https://pjax.cn/content/templates/FLY/js/checkcode.php"]').attr('title', '点击刷新验证码').click(function(){ this.src = this.src.replace(/\?.*$/, "") +'?'+ new Date().getTime();});
}
function pjaxcn() {
	try {
		ajaxcheckcode()
